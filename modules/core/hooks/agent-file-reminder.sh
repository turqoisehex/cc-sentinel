...
if [[ "$AGENT_TYPE" == "Explore" || "$AGENT_TYPE" == "Plan" ]]; then
  ...
  if ! echo "$PROMPT" | grep -qiE "(write|save|output).*(file|findings|results|report)|verification_findings"; then
    echo '{"additionalContext": "AGENTS WRITE TO FILES: This agent prompt does not appear to include instructions to write findings to a file. Every agent MUST write findings to disk (e.g., verification_findings/agent_*.md). Add file-writing instructions to the prompt, or plan to save results yourself after the agent returns."}'
  fi
fi
