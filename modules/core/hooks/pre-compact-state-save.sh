...
if [[ "$HAS_TASK" == "true" ]]; then
  MSG="COMPACTION IMMINENT (trigger: ${TRIGGER}). MANDATORY: Update your state file (CURRENT_TASK_chN.md if channeled, or CURRENT_TASK.md if unchanneled) NOW with: (1) Which plan step you just completed, (2) Which step is next, (3) Any uncommitted changes — describe them precisely, (4) Key decisions made this session, (5) Anything a fresh session needs that is not already in the file, (6) AGENT IDS AND OUTPUT FILES — list every background agent still running, its purpose, and the file path it is writing to (agent IDs do NOT survive compaction — the output file paths are the only way the next session can find results), (7) MEMORY.md — if any facts changed this session (counts, file paths, decisions, patterns), update MEMORY.md now; stale memory causes bad decisions in future sessions. Then commit via channel_commit.sh (or git commit if commit-enforcement is not installed). A fresh session will read ONLY CLAUDE.md, MEMORY.md, and the state files below — everything else is lost.${NL}${NL}YAML FRONTMATTER: Update the YAML block at the top of your state file with:${NL}- goal: one sentence describing the session objective${NL}- now: which step is currently in progress${NL}- done_this_session: list of completed items with file paths${NL}- decisions: key choices made this session${NL}- next: what the next session should do first${NL}- files_created/files_modified: paths touched this session${NL}${NL}Current state files for reference (update yours, do not start from scratch):${NL}${TASK_CONTENT}"
else
  MSG="COMPACTION IMMINENT (trigger: ${TRIGGER}). Write current work state to CURRENT_TASK.md (create it from the template if it does not exist). Include: what was done, what remains, decisions made, current approach, uncommitted changes. Also update MEMORY.md if any facts changed this session. A fresh session will have ZERO context from this session.${NL}${NL}YAML FRONTMATTER: Update the YAML block at the top of your state file with:${NL}- goal: one sentence describing the session objective${NL}- now: which step is currently in progress${NL}- done_this_session: list of completed items with file paths${NL}- decisions: key choices made this session${NL}- next: what the next session should do first${NL}- files_created/files_modified: paths touched this session"
fi
