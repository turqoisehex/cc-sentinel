...
| `/squad full` | All changes since session start (find boundary in CT or `git log --oneline -20`). |
| `/squad last` | `HEAD~1..HEAD` |
| `/squad last N` | `HEAD~N..HEAD` |
| `/squad since <ref>` | `<ref>..HEAD` + uncommitted |
| `/squad on <files>` | Specific file(s) only. Comma-separated or glob. |
...
Launch all 5 agents in parallel (`run_in_background: true`). Each agent writes to its output file in the squad directory.
