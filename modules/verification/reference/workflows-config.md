...
| `build-gates` | object | (skill defaults) | Optional. Per-project mechanisms the `/3` per-task deterministic VERIFY needs: `{ src, test, ext, diffScan?, gates: [{name, cmd}] }`. `src`/`test`/`ext` = the lens-4/5 grep roots + source extension; `diffScan` (optional) = the project's changed-field detector for lens 5 (ABSENT → lens 5 falls back to the path-scoped diff grepped for field-shape changes, a generic language-agnostic detector — never assumed "no fields changed"); `gates[]` = the cheap-gate `{name, cmd}` pairs (each `name` maps to the verdict-file `source: gate-<name>` id). NESTED object → schema-aware parsing. Absent → lenses 4/5 run with the skill-default `src`/`test`/`ext`, cheap-gate tier degrades to lenses 4/5 only (graceful default, NO warning, NEVER PARSE-FAIL). Present with `src`/`test`/`ext` but an OMITTED `gates` sub-list → same graceful cheap-gate degrade, NO warning (a valid minimalist config). Malformed (wrong shape — e.g. `gates` present but not an array of `{name,cmd}`, or `src`/`test`/`ext` missing) → graceful-default-WITH-warning (NOT a hard PARSE-FAIL, NOT a silent OFF). |
...
<!-- build-gates: { src: "<source-tree-root>", test: "<test-tree-root>", ext: "<source-file-ext>", diffScan: "<project-supplied diff command that detects data-model field changes>", gates: [ { name: "<gate-name>", cmd: "<lint/analyze command>" }, { name: "<gate-name>", cmd: "<test command>" } ] }  # optional /3 per-task verify mechanisms; each project supplies its own LANGUAGE-AGNOSTIC values (src/test roots, source extension, a field-change diff scanner, and the cheap-gate name/cmd pairs); absent = skill defaults, malformed = graceful-default-with-warning, never PARSE-FAIL -->
