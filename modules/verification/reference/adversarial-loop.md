...
project- or stack-specific idioms in engine logic; project mechanisms are supplied by
the phase skill.
...
```
{
  round,
  lensStatus[],               // each lens: ran | found-N | errored | carried-forward
  llmFanOutAgentIds[],        // agent ids for the LLM-backed lanes (the active finderSet's declared LLM lanes)
  deterministicReceiptIds[],  // signed receipts for the active finderSet's declared deterministic lanes
  coverageReceipt,
  scopeHash,
  pending,
...
This is the named `/5` finderSet. All nine lenses apply universally — no project-specific mechanisms are baked into the engine. The concrete mechanism (e.g., which DI-graph file, which test command) is supplied by the phase skill. Concrete examples appear only as parentheticals.
...
**Mutation-target + cleanup guards (DETERMINISTIC, both required):** Before mutating, the check MUST verify the mutation target path is NOT a primary-tree path — compare the resolved target path prefix against the primary-tree root; a primary-tree match is a **lens-6 engine-error FAIL emitted before any mutation** (the check never mutates the primary tree, even by misconfiguration). After the mutate-and-run, the check MUST verify the temp copy/worktree was removed — `stat` the temp path; a still-present temp artifact is a **lens-6 cleanup FAIL emitted to the findings before any other lens runs in the same round** (a leaked temp copy inside the project tree would otherwise enter the engine's under-declaration scope for a later round). A mis-targeted mutation or a failed cleanup is a FAIL, never a silent pass.
...
Verified via the **project-provided DI-graph probe** (supplied by the phase skill, not baked into the engine). *Example: a live non-null assertion that the production dependency-injection container actually wired the declared dependency.* A mock-covered constructor graph looks wired but is not.
...
Split from Lens 7 because call-site-only defects have no constructor field to null-assert — a single wiring lens is structurally blind to this class. Verified via the **project-provided entry-point probe** (supplied by the phase skill). *Example: a call-site-only gap that a unified wiring lens missed because it had no constructor field to null-assert.*
...
<!-- The trailing period in `## 3c.` is intentional — the selftest awk lens-counter anchors on it (`## 3c\.`). Do not drop the dot when editing this heading. -->

This is the named `/3` finderSet — the lenses the ENGINE runs **only at the checkpoint** (a commit-group / phase boundary, FIRED BY THE SKILL, never by the engine mid-task). It adapts the `/5` lenses (`## 3`) to **pre-commit, build-time** verification. It is **NOT** the per-task verify: the per-task skill stage runs only the deterministic lanes `{4,5}` + cheap gates (see `build/SKILL.md § The serial per-task loop` for the per-task cadence). `lensCount: 5`. Lenses are universal; concrete mechanisms (test command, source tree, consumer grep, diff-scan) are **supplied by the build skill** (the `build-gates` config key) — no language/framework-isms in the engine.
...
**Mutation-target + cleanup guards (DETERMINISTIC, both required):** Before mutating, the check MUST verify the mutation target path is NOT a primary-tree path — compare the resolved target path prefix against the primary-tree root; a primary-tree match is a **lens-4 engine-error FAIL emitted before any mutation** (the check never mutates the primary tree, even by misconfiguration). After the mutate-and-run, the check MUST verify the temp copy/worktree was removed — `stat` the temp path; a still-present temp artifact is a **lens-4 cleanup FAIL emitted to the findings before any other lens runs in the same round** (a leaked temp copy inside the project tree would otherwise enter the `filesWritten[]` under-declaration scope for a later check). A mis-targeted mutation or a failed cleanup is a FAIL, never a silent pass.
...
**Pipeline-log entry schema (`pipeline-log.jsonl` — the PARENT appends one entry per stage; the globally-monotonic `seq` is the ordering substrate):**

```json
{ "task": "<taskId>", "stage": "PRODUCE", "seq": <n>, "group": "<commit-group-id>", "baselineRef": "<git stash create object id>" }   // PRODUCE entry — carries the task's commit-group id AND the captured baselineRef (persisting baselineRef redundantly with the result-FILE so a resumed /3 can reconstruct the per-task diffs from either artifact)
{ "task": "<taskId>", "stage": "VERIFY",  "seq": <n>, "group": "<commit-group-id>" }                    // VERIFY entry — carries the task's commit-group id (so group membership of each VERIFY is read from the log itself); no baselineRef (the parent already persisted it on the matching PRODUCE entry)
{ "stage": "CHECKPOINT_ENGINE_START", "seq": <n>, "group": "<commit-group-id>" }                         // checkpoint entry — no `task`; appended when the skill fires the checkpoint engine; the `group` id names the commit-group this checkpoint covers
```

Every entry carries the commit-group id, so the per-checkpoint ordering check is provable **entirely within the pipeline-log** with NO external plan knowledge: for each commit-group G, every per-task `VERIFY` entry whose `group` equals G has `seq` less than the `CHECKPOINT_ENGINE_START` entry whose `group` equals G. Group membership of each PRODUCE/VERIFY is read from its own `group` field — the parent stamps it at write time from the task's commit-group per the plan's commit-group boundary definition. For the one-commit-per-`/3` default cadence there is a single group and the check reduces to "every VERIFY seq < the lone CHECKPOINT_ENGINE_START seq."

The PARENT writes ALL entries (including for `[SONNET]` tasks); the subagent never writes the log. Only the PRODUCE entry carries `baselineRef` (the parent owns baseline capture, §3.6 result-FILE `baselineRef`).
...
```json
{
  ...
  "baselineRef":   "<the git stash create object id captured by the parent at PRODUCE(N) start>"  // required — the PERSISTED per-task baseline; a resumed /3 reconstructs `git diff <baselineRef>` (full + path-scoped) from this rather than falling back to cumulative HEAD. SINGLE-WRITER MECHANISM: the PARENT OWNS baseline VALUE-capture (it runs `git stash create` at PRODUCE(N) start and injects the object id into the PRODUCE dispatch); the result-FILE FIELD is written by whoever writes the result-FILE — the producer (subagent in native mode, listener in duo mode) ECHOES the parent-injected baselineRef verbatim into the result-FILE it writes. For a [PARENT]-executed task the parent writes the result-FILE directly (it is both value-owner and file-writer). The parent does NOT separately patch the field post-write; the producer's echo is authoritative, and the parent cross-checks it against the value it injected (a mismatch = result-FILE FAIL). The pipeline-log PRODUCE entry's baselineRef is written by the parent directly (the parent owns the log in both modes), giving a parent-authored second copy for the resume path.
}
```
...
```json
{
  ...
  "scopeHashChecked": "<the scopeHash the verdict was computed against>"  // required — a content digest over task N's filesWritten[] (deferral-ledger and audit writes excluded, mirroring ## 6.1 exclusions). SINGLE-WRITER MECHANISM: the PARENT COMPUTES the value at the PRODUCE(N)→VERIFY(N) handoff (using the same content-digest function the engine uses, ## 6) and passes it INTO the VERIFY dispatch; the verifier WRITES that parent-passed value into this field of the verdict-FILE it authors. The verifier does not compute its own digest; the parent does not patch the field post-write. The parent then re-computes the digest of the tree the verifier actually ran against and REJECTS any verdict-FILE whose scopeHashChecked ≠ that re-computed value (the cross-check the §5 parent-enforcement rule acts on), re-running VERIFY(N) on a mismatch.
}
```
...
**Known limitation of `scopeHashChecked` (narrow — create-then-delete within one PRODUCE):** `scopeHashChecked` is a digest over `filesWritten[]`, and the under-declaration check reads the baseline full-diff. A producer that CREATES then DELETES an undeclared file within a single PRODUCE is net-invisible to both: the file is absent from the baseline full-diff (created and removed since the baseline) and leaves `filesWritten[]` unchanged. This is a narrow deliberate-evasion edge case; the known-limitation note is the fix (no mechanism overhaul) — the primary protection (an under-declaration FAIL for any producer-touched undeclared file that PERSISTS through VERIFY) is sound.
