...
**Selftest-activation rule (resolves the enable-gate deadlock — self-enable then verified-revert):** the engine path is config-OFF unless `"/3"` is in `enabled-phases`, but the HARD-BLOCK enable gate (obligation 6 / Step 7.7) forbids adding `"/3"` to the LIVE `enabled-phases` until this selftest PASSES — a circular deadlock if the selftest needs the engine path. The `/3`-mode selftest therefore uses a **self-contained `temp-enable → run → verified-revert` mechanism**: at `/3`-mode start it sets `workflows_enabled: true` AND adds `"/3"` to `enabled-phases` in the config it reads (a scoped, in-run enablement — NOT the live operator enablement obligation 6 gates), runs the assertions on the engine path, then **REVERTS both** (removes `"/3"` from `enabled-phases` and restores the prior `workflows_enabled` value). This self-contained temp-enable-then-verified-revert is what lets the selftest reach the engine path WITHOUT the operator first performing the live enablement obligation 6 forbids until the selftest passes. (This is a `/3`-mode generalization of the `/4`-mode spend-gated activation, made self-reverting; the `/4`-mode activation itself is an explicit OPERATOR step and is NOT self-reverting.) **The revert MUST be verified before any commit** — the selftest re-reads the config after revert and asserts `"/3"` is absent from `enabled-phases` and `workflows_enabled` is restored; a non-reverted config is a selftest FAIL (leaving `/3` live would bypass the obligation-6 hard-block). **Operator-repair on a non-reverting config (the revert write itself failed):** a bare FAIL is insufficient — when the revert is not verified, the selftest MUST emit, before stopping, the exact manual repair the operator must perform: `CONFIG REPAIR REQUIRED: the selftest temp-enabled /3 but could not verify the revert. Manually remove "/3" from enabled-phases and restore workflows_enabled to <prior-value> in <config-path> before running /3 — the live config is currently in the temp-enabled state and would bypass the obligation-6 hard-block.` The operator needs the repair action, not just the failure signal.
...
```bash
# The heading is `## 3c.` WITH a trailing period — the dot is INTENTIONAL. Both patterns below
# dot-anchor on it (`## 3c\.`) so the existence grep and the lens-counting awk stay consistent: a
# future heading written without the dot would fail BOTH, never silently passing one and breaking the other.
...
**(iv) Stale-receipt REJECTION for the 2-element deterministic set `{4,5}`** (RECEIPT-INVALIDATION + re-run-trigger; NOT a full lane-4 throwaway-worktree mutate-run — the throwaway worktree is used only inside lens 4's test-honesty check, not as the mutation vehicle here): run one checkpoint round (receipts signed at PRIMARY-tree `scopeHash` H1), then mutate a **PRIMARY-TREE in-scope file** (the mutation vehicle is the primary tree so `scopeHash` changes to H2 ≠ H1). Assert BOTH lane-4 (`4/test-honesty`) and lane-5 (`5/consumer-preflight`) receipts are REJECTED and re-run (NOT carried forward against H2 — dry is NOT credited against stale receipts; `## 6.3`/`## 6.4` for a 2-element set, a new cardinality between `/4`'s 1 and `/5`'s 3). After the assertion, **revert the primary-tree mutation** (the mutation is a test vehicle; leaving it would corrupt subsequent build steps). **Revert mechanism (record-then-restore):** BEFORE mutating, record the in-scope file's exact content; after the assertion, restore that recorded content via an Edit/Write call (do NOT use `git checkout` — it could reset other in-flight working-tree changes). **Selftest-FAIL gate (CRLF-tolerant comparison):** after restore, re-read the file and assert its content equals the recorded original — but compare **line-ending-normalized** (apply `tr -d '\r'` to BOTH the recorded original and the restored content before comparing), so a line-ending-only difference introduced by the Edit/Write tool on a Windows host does NOT false-fire the gate. A line-ending-only delta MUST NOT trigger the FAIL; only a substantive content difference does. If the revert did NOT complete (a substantive, CRLF-normalized content mismatch), the selftest MUST emit a selftest FAIL and STOP — it does not proceed to further assertions, so a corrupted (mutation-left) working tree can never be carried into a commit. This 2-element-set assertion is the explicit gate that the `{4,5}` cardinality is selftest-demonstrated before `/3` goes live.
...
**(vi) checkpoint engine run fires AFTER all per-task VERIFY entries:** the skill writes a `{ stage: CHECKPOINT_ENGINE_START, seq, group }` entry to the pipeline-log when it fires the checkpoint engine, AND every per-task `PRODUCE`/`VERIFY` entry carries its own `group` id; assert ENTIRELY WITHIN the pipeline-log that, **for each commit-group G, every per-task `VERIFY` entry whose `group` equals G has `seq` less than the `CHECKPOINT_ENGINE_START` entry whose `group` equals G** (group membership is read from each entry's OWN `group` field — NO external plan knowledge; for the default one-commit-per-`/3` cadence there is one group and one checkpoint, so this reduces to every VERIFY seq < the lone CHECKPOINT_ENGINE_START seq). Because the per-task entries carry `group` too, the check is provable purely from the log for a load-bearing mid-`/3` split that fires more than one checkpoint (without per-entry `group`, a first group's VERIFY entries would spuriously satisfy a second group's later CHECKPOINT_ENGINE_START seq, masking an ordering violation in the second group). This is a single-log `seq` ordering check, NOT a cross-log comparison with the engine journal (the journal has no comparable global `seq`; it separately proves the checkpoint RAN via (ii)/(iii), while the pipeline-log proves the ORDER). Catches an implementer who wired the engine (or LLM lenses 1–3) as a per-task interrupt instead of a single skill-fired checkpoint pass (violating WIP §7 and the §3.4 cadence division).
...
**(ix) design-gap → RETURN-TO-`/2` halt:** the fixture seeds a producer **deferral** — a synthetic task whose result-FILE carries a non-empty `deferrals[]` (a design decision the `/2` plan did not resolve). Assert that the per-task verifier emits `verdict: RETURN_TO_2` (NOT PASS, NOT a benign WARN-continue) and that the `/3` loop **HALTS** (does NOT advance to the next PRODUCE). **Mechanize the two halt-observables directly (do NOT infer "no commit" from the absence of a PRODUCE(N+1) entry):**
- **(a) commit-did-not-fire (git HEAD before/after):** record `git rev-parse HEAD` BEFORE the fixture run and again AFTER the RETURN_TO_2 halt; assert the two are byte-equal (no new commit object was created). A loop that halts but still fires the single `/3` commit would pass an absence-of-PRODUCE(N+1) check yet fail this direct comparison.
- **(b) CT `## Return to /2` section written (grep):** read the CT file at the channel path after the halt and `grep` for the `## Return to /2 — design-decision-gap` section heading; assert ≥1 match. The CT-write is verified by an explicit file read, not inferred.

This proves the strictly-execution discipline: a `/2`-incompleteness halts-and-returns rather than being conservative-defaulted or parked for `/3`-end approval. A builder who lets `/3` take a default and continue (the removed pre-RQ-3 behavior) fails this.
...
The in-harness `--selftest` RUN is the Task 7 Step 7.6 acceptance gate. The `/5`-mode sub-block runs under the already-enabled `/5` finderSet; the `/4`-mode sub-block runs once the operator adds `"/4"` to the live `enabled-phases` (the deliberate spend-gated activation step); the **`/3`-mode sub-block** runs via the **self-contained temp-enable → run → verified-revert** mechanism (it does NOT require the operator to first add `"/3"` to the live `enabled-phases` — it self-enables `/3` for the run and reverts before any commit, the revert verified). **The `/3`-mode sub-block IS part of the acceptance gate: it is the `/3` enable-gate** — `/3` MUST NOT be added to the live `enabled-phases` until this `/3`-mode block PASSES (the HARD-BLOCK enable gate). All three per-mode sub-blocks run in ONE `--selftest` invocation; the global PASS contract spans `/5`, `/4`, AND `/3`.
