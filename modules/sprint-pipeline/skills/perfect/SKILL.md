...
- **Terminate at the consumer.** Every spec claim must be traced to the runtime component that READS and USES the field — not just to the declaration. A field in `ExerciseDefinition.parameters` that no engine/widget reads is a dead declaration, not a fulfilled spec.
- **Silent-fallback scan.** Grep consumers for `?? ` and `|| ` near each consumption site. If `params['fieldX'] ?? 4` exists and no test proves the exercise declares `fieldX`, that's a silent override — FAIL.
...
**Audit-pointer rule (MANDATORY in dispatch prompt):** Both audit files are durable artifacts — they will be re-read across every subsequent verification round in this sprint. Per `~/.claude/reference/audit-pointer-rules.md`, cite code via symbolic addresses (`file.dart :: ExerciseDefinition(id: 'x').field`), NOT line numbers. Do not emit `file:L\d+`, `L\d+-L\d+`, `line N`, or `~LN` forms. Quote verbatim snippets where locality matters — the quote is self-locating via grep. Include this paragraph verbatim in the Sonnet dispatch prompt.
