...
- **Terminate at the consumer.** Every spec claim must be traced to the runtime component that READS and USES the field — not just to the declaration. A field in `ModelDefinition.parameters` that no engine/widget reads is a dead declaration, not a fulfilled spec.
- **Silent-fallback scan.** Grep consumers for `?? ` and `|| ` near each consumption site. If `params['fieldX'] ?? 4` exists and no test proves the entity declares `fieldX`, that's a silent override — FAIL.
