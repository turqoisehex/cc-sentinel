...
`/grill` all `/perfect` work product. Fix -> test -> repeat until clean or 5 rounds. Batch all grill fixes into one commit before Phase 3. **Commit protocol:** see `.claude/reference/commit-protocol.md` — use `git diff HEAD -- <files>` for verifier input, never `git diff --cached`, never pre-stage.

## Phase 3: Squad Loop (max 3 rounds)

DELEGATE 5-agent squad from `.claude/reference/verification-squad.md`. All agents must PASS or WARN. FAIL: fix -> re-run failed agents only. Max 3 rounds.
