...
```bash
# Forbidden patterns — substitute YOUR host project's identifiers, then run ALL:
grep -niE "<your-project-name>" "$file"                                 # project name, any case
grep -nE "<YourDomainClass>|<YourEntityType>|<YourConfigType>" "$file"  # domain class / type names
grep -nE "<your/spec/dir>/|<your/source/layout>/" "$file"               # project-specific doc/source paths
grep -nE "<PROJECT>_CHANNEL|<PROJECT>_LISTENER" "$file"                  # project-name env vars
grep -n "SPRINT_CHECKLIST\|COMPREHENSIVE_IMPLEMENTATION_PLAN" "$file"    # cc-sentinel artifact names (template-exempt)
grep -n "decisions_ch[0-9]" "$file"
grep -nE "<your-domain-vocabulary>" "$file"                             # app-domain nouns specific to your product
grep -n "Sprint\s*1[0-9]" "$file"
grep -nE "[0-9]{4}-[0-9]{2}-[0-9]{2}" "$file"  # specific dates
grep -nE "<your-name>|<your-email-handle>|<your-home-path>" "$file"     # personal identity
```
...
|---|---|
| Your source-layer paths (`<lib/...>`, `<src/...>`) | "your presentation/domain/data layer (e.g., `src/ui/`, `lib/domain/`, `app/models/`)" |
| Your domain class / type names | "any entity class, schema definition, config type, or domain model" |
| `<your/spec/dir>/specs/<date>-...` | "your project's spec file" or generic path placeholder |
| Single-stack-specific patterns | Generic multi-stack examples (Python/TS/Go/Rust) |
| Specific app-domain references | Generic domain examples |
...
| `<PROJECT>_LISTENER`, `<PROJECT>_CHANNEL` | Remove entirely — project-name env vars are never universal |
...
```bash
cd <cc-sentinel-repo>
git diff HEAD -- . ':!modules/*/templates/*-template.md' | grep -i "wakeful\|ExerciseDefinition\|superpowers\|WAKEFUL_CHANNEL\|SPRINT_CHECKLIST\|breathing.engine\|<your-name>"
```
...
6. **Assuming project-name env vars are "fine"** — a `<PROJECT>_CHANNEL`-style var is obviously project-specific
